...
;; straight.el replaces package.el entirely:
;; - `:ensure t` from the old config is now handled by `:straight`
;; - `straight-use-package-by-default' means every `use-package' is
;;   treated as `:straight t` unless explicitly `:straight nil`
(straight-use-package 'use-package)
(setq straight-use-package-by-default t)

;;; agent-shell -- LLM agent shells in Emacs via ACP (Agent Client Protocol)
;; Installed by straight (straight-use-package-by-default t).
;;
;; We use the Pi agent with OpenRouter as the model provider.
;; Pi manages its OWN credentials natively via /login (stored in
;; ~/.pi/agent/auth.json, already set to OpenRouter), so no API-key
;; handling is needed here. agent-shell's pi client runs `pi-acp'
;; (installed via `npm install -g pi-acp') and passes no key env vars.
;;
;; Start with:  M-x agent-shell-pi-start-agent
(use-package agent-shell
  :config
  ;; agent-shell-pi-acp-command defaults to ("pi-acp"), which is on PATH.
  ;; Pi reads its own saved OpenRouter key from ~/.pi/agent/auth.json.
...
;;   '(agent-shell company eglot exec-path-from-shell flycheck ghostel gptel graphql-mode groovy-mode helm-projectile
;;		 lorem-ipsum magit markdown-mode mcp multiple-cursors prettier-js try typescript-mode uuidgen wgrep-helm)))
;;(custom-set-faces
